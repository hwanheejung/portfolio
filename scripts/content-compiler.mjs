...
function resolveAssetUrl(collection, id, assetPath) {
  ...
  return ["/_content", collection, id, relativePath]
    .filter(Boolean)
    .join("/");
}
...
export const content = ${JSON.stringify(manifest, null, 2)} as const satisfies ContentManifest;
...
export async function compileContent({
  ...
  const articleEntries = (await readdir(ARTICLES_ROOT, {
    withFileTypes: true,
  }))
